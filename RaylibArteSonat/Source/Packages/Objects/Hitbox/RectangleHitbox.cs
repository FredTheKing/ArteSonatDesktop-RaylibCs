...
public class RectangleHitbox(Vector2 position, Vector2 size, Color color) : HitboxTemplate(position, color)
{
  ...
  {
    Vector2 nonref_position = _hitbox_position;
    _hitbox_click_hover = Raylib.CheckCollisionPointRec(Raylib.GetMousePosition(), new Rectangle(nonref_position, _hitbox_size));
  }
