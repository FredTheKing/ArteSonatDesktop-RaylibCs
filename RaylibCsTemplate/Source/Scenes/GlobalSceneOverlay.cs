...
public class GlobalSceneOverlay(Registry registry)
{
  ...
  public void Update()
  {
    if (registry.GetShortcutManager().GetPressedKey() == KeyboardKey.KEY_ONE) registry.GetSceneManager().ChangeScene("TestSceneOne");
    else if (registry.GetShortcutManager().GetPressedKey() == KeyboardKey.KEY_TWO) registry.GetSceneManager().ChangeScene("TestSceneTwo");
  }
